...
namespace DataIntegration.table
{
    ...
    public class TideTableTrans
    {
        ...
        public string GetSelectSql(string stcd, DateTime bgtime, DateTime edtime)
        {
            ...
            return string.Format("select stcd, tm, tdz from ST_TIDE_R where stcd='{0}' and tm>'{1}' and tm<'{2}'", stcd, bg_tmp, ed_tmp);
        }
        ...
        /// <returns></returns>
        public int InsertOrUpdateData(string stcd, DateTime tm, double tdz, TConnect conn_des)
        {
            ...
            string tdz_str = tdz == -999999 ? "null" : tdz.ToString(); // 可能为null值

            string check_sql = string.Format("select tdz from ST_TIDE_R where stcd='{0}' and tm='{1}'", stcd, tm_str);
            ...
            {
                // 不存在，需要插入
                exec_sql = string.Format("insert into ST_TIDE_R(stcd, tm, tdz, flag) values('{0}','{1}',{2},0)", stcd, tm_str, tdz_str);
            }
            else
            {
                DataRow row = dt.Rows[0];
                // 已存在，则先比较，drp雨量不同则进行更新
                double tdz_old = row.IsNull("tdz") ? -999999 : double.Parse(row["tdz"].ToString());
                if (tdz_old == tdz ) return 1; // 数据是一样的，不用处理
                // 不一样则进行更新
                exec_sql = string.Format("update ST_TIDE_R set tdz={0} where stcd='{1}' and tm='{2}'", tdz_str, stcd, tm_str);
            }
        ...
        public DateTime ExecTableTrans(List<string> stcds, DateTime bgtime, DateTime edtime, TConnect conn_source, TConnect conn_des)
        {
            ...
            while (TodoTransQueue.Count>0)
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    double tdz = row.IsNull("tdz") ? -999999 : double.Parse(row["tdz"].ToString());

                    int change = InsertOrUpdateData(stcd, tm, tdz, conn_des);
                    count_trans += change;
