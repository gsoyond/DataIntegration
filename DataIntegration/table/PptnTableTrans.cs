...
namespace DataIntegration.table
{
    ...
    public class PptnTableTrans
    {
        ...
        public string GetSelectSql(string stcd, DateTime bgtime, DateTime edtime)
        {
            ...
            return string.Format("select stcd, tm, drp from ST_PPTN_R where stcd='{0}' and tm>'{1}' and tm<'{2}'", stcd, bg_tmp, ed_tmp);
        }
        ...
        /// <returns></returns>
        public int InsertOrUpdateData(string stcd, DateTime tm, double drp, TConnect conn_des)
        {
            ...
            string drp_str = drp == -999999 ? "null" : drp.ToString(); // drp可能为null值
            string check_sql = string.Format("select drp from ST_PPTN_R where stcd='{0}' and tm='{1}'", stcd, tm_str);
            ...
            {
                // 不存在，需要插入
                exec_sql = string.Format("insert into ST_PPTN_R(stcd,tm,drp,flag) values('{0}','{1}',{2},0)", stcd, tm_str, drp_str);
            }
            else
            {
                DataRow row = dt.Rows[0];
                // 已存在，则先比较，drp雨量不同则进行更新
                double drp_old = row.IsNull("drp") ? -999999 : double.Parse(row["drp"].ToString());
                if (drp_old == drp) return 1; // drp雨量是一样的，不用处理
                // 不一样则进行更新
                exec_sql = string.Format("update ST_PPTN_R set drp={0} where stcd='{1}' and tm='{2}'", drp_str, stcd, tm_str);
            }
        ...
        public DateTime ExecTableTrans(List<string> stcds, DateTime bgtime, DateTime edtime, TConnect conn_source, TConnect conn_des)
        {
            ...
            while (TodoTransQueue.Count>0)
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    double drp = row.IsNull("drp") ? -999999 : double.Parse(row["drp"].ToString());

                    int change = InsertOrUpdateData(stcd, tm, drp, conn_des);
                    count_trans += change;
